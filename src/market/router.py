...
async def update_koefs(match_id: str, market: str, response: Response):
    ...
    if 'markets' not in decoded_koef.keys():
        return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST, content={'msg': f"No match with id {match_id}"})
    ...
    return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST, content={'msg': f"No market {market} in match {match_id}"})
